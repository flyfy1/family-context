...
Android 已沿用同一组 `en` / `zh` 语言代码并提供独立设置，见 [Android 语言设置 V1](./android-language-settings-v1.md)。自动检测系统语言、第三种语言、历史内容翻译和后端所有错误文案本地化不在本版本范围内。
...
Android 客户端沿用 `en` / `zh` 两个语言代码，首次默认英文，并持久保存用户选择。向日报或睡前故事生成接口发请求时应明确提交当前 App 语言；不能依赖后端默认值来猜测用户语言。
