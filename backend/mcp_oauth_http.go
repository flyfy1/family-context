...
</style></head><body><main class="card"><h1>Connect Family Daily</h1><p class="client"><strong>{{.ClientName}}</strong> wants access to this member's Space and context, plus family-visible updates.</p>{{if .Error}}<p class="error">{{.Error}}</p>{{end}}<form method="post" action="/oauth/authorize">{{range $key, $value := .Hidden}}<input type="hidden" name="{{$key}}" value="{{$value}}">{{end}}<label>Member or MCP session token<input type="password" name="member_access_token" autocomplete="current-password" required autofocus></label><button type="submit">Allow this member's Family Daily access</button></form><p class="boundary">The client can only use <code>{{.Resource}}</code>. It cannot access another member's private content, the storage root, or a shell.</p></main></body></html>`))
