...
public final class PhotoSyncWorker extends Worker {
    ...
    public Result doWork() {
        ...
        if (!PhotoSync.hasImageAccess(context)) {
            setStatus(prefs, "需要允许照片访问，才能继续同步");
        ...
        if (!PhotoSyncWindow.isValidDays(lookbackDays)) {
            setStatus(prefs, "同步暂停：照片回溯天数设置不正确");
        ...
        try (Cursor media = resolver.query(collection, projection, selection, args, order)) {
            if (media == null) throw new IllegalStateException("无法读取系统相册");
            ...
            while (media.moveToNext()) {
                ...
                setStatus(prefs, "正在同步第 " + (uploaded + 1) + " 张照片……");
                try (InputStream input = resolver.openInputStream(uri)) {
                    if (input == null) throw new IllegalStateException("无法打开照片");
            ...
            setStatus(prefs, "照片权限已变化，请重新允许访问");
            return Result.success();
            ...
            if (httpError.statusCode == 400 || httpError.statusCode == 401 || httpError.statusCode == 403) {
                setStatus(prefs, "同步暂停：请检查 NAS 地址、成员令牌或照片格式");
                return Result.failure();
            }
            setStatus(prefs, "网络或服务暂时不可用，稍后自动重试");
            return Result.retry();
        } catch (Exception error) {
            setStatus(prefs, "同步暂时中断，稍后自动重试");
            return Result.retry();
        }

        String summary = uploaded == 0 ? "最近 " + lookbackDays + " 天的照片已是最新"
                : "已同步最近 " + lookbackDays + " 天内的 " + uploaded + " 张照片到私人 NAS 空间";
        if (skipped > 0) summary += "，跳过 " + skipped + " 个不支持或过大的文件";
