...
function Topbar({ route, members, currentMember, onMemberChange }) {
  ...
  return <header className="topbar">
    ...
    {members.length > 0 && <label className="member-switcher"><span>当前身份</span><select value={currentMember?.id || ""} onChange={event => onMemberChange(event.target.value)}>{members.map(member => <option key={member.id} value={member.id}>{member.name}{member.role === "elder" ? " · 老人" : ""}</option>)}</select></label>}
  </header>;
}
...
function Settings({ api, config, setConfig, members, refresh, notify }) {
  ...
  return <div className="settings-page">
    ...
    <section className="settings-section card"><div><p className="eyebrow">MEMBER SPACES</p><h2>家庭成员</h2><p className="muted-copy">每位成员会获得独立的本地文件目录。老人角色可以使用专用的大按钮页面。</p></div><div><div className="member-list">{members.map(member => <div className="member-list-item" key={member.id}><Avatar member={member} /><div><strong>{member.name}</strong><span>{member.role === "elder" ? "老人模式" : "家庭成员"} · 独立 Space</span></div><code>{member.id.slice(0, 8)}</code></div>)}</div><form className="add-member" onSubmit={addMember}><input value={newMember.name} maxLength="30" onChange={event => setNewMember({ ...newMember, name: event.target.value })} placeholder="成员称呼" required /><select value={newMember.role} onChange={event => setNewMember({ ...newMember, role: event.target.value })}><option value="member">普通成员</option><option value="elder">老人</option></select><div className="color-picker">{colors.map(color => <button type="button" aria-label={`选择颜色 ${color}`} key={color} className={newMember.color === color ? "active" : ""} style={{ background: color }} onClick={() => setNewMember({ ...newMember, color })} />)}</div><button className="primary-button" disabled={busy}>{busy ? "正在创建…" : "+ 添加成员"}</button></form></div></section>
