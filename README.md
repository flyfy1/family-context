...
1. 在首页点击“连接 NAS”；
2. 填写手机可访问的 Family Daily 服务地址、该成员自己的令牌，以及需要回溯同步的天数；
3. 允许访问全部照片，或在 Android 14+ 只选择希望同步的照片；
4. 默认从最近 3 天的照片开始。App 会立即同步该滚动时间窗内的已有照片，并在联网时由 Android 大约每 15 分钟安排一次后台增量同步；点击“立即同步”可手动触发。

回溯范围可设置为 1–3650 天，以照片拍摄时间为准；缺少拍摄时间时使用照片加入系统相册的时间。扩大范围后 App 会从新的时间窗重新补扫，服务端幂等键确保已有记录不会重复创建。缩小范围或调整范围不会删除 NAS 中已经保存的照片。

同步只复制照片，不会因为手机端删除而删除 NAS 原件。每张照片使用稳定的设备 ID 和 MediaStore ID 幂等上传；断网重试不会生成重复记录。成员令牌保存在 App 私有数据中，不写入日志或照片元数据。

当前自动同步 JPEG、PNG、WebP 和 GIF。HEIC/HEIF 会显示为跳过，需先扩展后端格式支持。Android 的后台调度会受到省电和 Doze 限制，不能保证严格实时；打开 App 会立即补同步。
